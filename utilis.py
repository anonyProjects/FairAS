...
def get_test_data(model_params, data_params):
    ...
    if dataset == 'ml-1m':
        ...
        test_data['cate'] = test_data['play_duration']
    return test_data
...
def get_saved_model_details(model_params, data_params):
    ...
    if dataset == 'black-fri':
        train_feats = ['user_id', 'item_id', 'gender', 'age', 'occupation', 'city', 'stay_years', 'martial_status',
                       'cate', 'subcate_1', 'subcate_2', 'price']
    if dataset == 'last-fm':
        train_feats = ['user_id', 'item_id', 'gender', 'age', 'country', 'signup_date', 'cate']
    if dataset == 'amazon':
        train_feats = ['user_id', 'item_id', 'price', 'cate']
    if dataset == 'kuai':
        train_feats = ['user_id', 'item_id', 'cate', 'onehot_feat0', 'onehot_feat1', 'onehot_feat2', 'onehot_feat3',
                       'onehot_feat4', 'onehot_feat5', 'onehot_feat6', 'onehot_feat7', 'onehot_feat8', 'onehot_feat9',
                       'onehot_feat10', 'onehot_feat11', 'onehot_feat12', 'onehot_feat13', 'onehot_feat14',
                       'onehot_feat15', 'onehot_feat16', 'onehot_feat17']
    if dataset == 'reasoner':
        train_feats = ['user_id', 'item_id', 'cate', 'age', 'gender', 'education', 'career', 'income', 'address']
...
def get_model(model_params, data_params):
    ...
    if model_name == 'fgcnn':
        model = model = FGCNN(linear_feature_columns, dnn_feature_columns, conv_kernel_width=(3, 2),
                              conv_filters=(2, 1), new_maps=(
                2, 2), pooling_width=(2, 2), dnn_hidden_units=(32,), dnn_dropout=0.5, )
    if model_name == 'edcn':
        model = EDCN(linear_feature_columns, dnn_feature_columns, task='binary')
        print('load EDCN model')
    # model.summary()
