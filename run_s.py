...
def local_search(test_info, feat_dict, temp_sample, score_target, direction, sampled_users_str, sampled_users,
    ...
    for s in test_info['sensitive_features']:
        ...
        orig_value = temp_sample[change_index]
        change_value_list = []
        if orig_value + 1 <= feat_dict[s] - 1:
            change_value_list.append(orig_value + 1)
        if orig_value - 1 >= 0:
            change_value_list.append(orig_value - 1)
        if orig_value + 2 <= feat_dict[s] - 1:
            change_value_list.append(orig_value + 2)
        if orig_value - 2 >= 0:
            change_value_list.append(orig_value - 2)
